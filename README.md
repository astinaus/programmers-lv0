...
function solution(a, d, included) {
  const array = new Array(included.length)
    .fill(0)
    .map((el, i) => (i === 0 ? el + a : el + a + d * i));
  return eval(array.filter((_, i) => included[i]).join('+'));
}
```
...
function solution(s) {
  ...
  for (let i = 0; i < s.length; i++) {
    s[i] !== 'Z' ? (answer += parseInt(s[i])) : (answer -= parseInt(s[i - 1]));
  }
  return answer;
}
```
